...
async function sendEmailInternal(to: string, subject: string, text: string, html?: string) {
  if (!transporter) {
...
export async function sendEmail(params: { to: string; subject: string; text: string; html?: string }) {
  return sendEmailInternal(params.to, params.subject, params.text, params.html);
}
...
export async function sendMentorApprovalEmail(params: { email: string; name: string; loginUrl: string }) {
  ...
  await sendEmailInternal(params.email, subject, body);
}
...
export async function sendFounderOTP(params: { email: string; name: string; otp: string }) {
  const subject = 'Your XENTRO Founder Login Code';
  const text = `Hi ${params.name},\n\nYour login code is: ${params.otp}\n\nThis code will expire in 10 minutes.\n\nIf you did not request this code, please ignore this email.\n\nBest regards,\nThe XENTRO Team`;
  const html = `
    <div style="font-family: -apple-system, BlinkMacSystemFont, 'Segoe UI', Roboto, sans-serif; max-width: 600px; margin: 0 auto; padding: 40px 20px;">
      <div style="text-align: center; margin-bottom: 40px;">
        <h1 style="color: #1a1a1a; font-size: 24px; margin-bottom: 10px;">Founder Login Code</h1>
        <p style="color: #666; font-size: 14px;">Enter this code to access your XENTRO dashboard</p>
      </div>
