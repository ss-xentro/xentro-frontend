...
export function HeroSection({ mentor, connectionStatus, btnConfig, onAction }: HeroSectionProps) {
	return (
				...
				<div className="flex-1 min-w-0">
					<div className="flex flex-col sm:flex-row sm:items-start justify-between gap-4">
						...
						<div className="flex gap-2 shrink-0">
							...
							<button onClick={onAction} disabled={btnConfig.disabled} className={`px-5 py-2.5 rounded-xl text-sm font-semibold border transition-colors ${btnConfig.className}`}>{btnConfig.label}</button>
						</div>
					</div>
				</div>
			</div>
