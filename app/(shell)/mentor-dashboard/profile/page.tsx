...
export default function MentorProfilePage() {
	...
	const addPricingPlan = () => {
		...
		setPricingPlans((prev) => [
			...prev,
			{ sessionType: "", duration: "", price: "", perks: [] },
		]);
	};
	...
	const updatePricingPlan = (
		idx: number,
		field: keyof Omit<PricingPlan, "perks">,
		value: string,
	) => {
		setPricingPlans((prev) =>
	...
	const removePerk = (planIdx: number, perkIdx: number) => {
		setPricingPlans((prev) =>
			prev.map((p, i) =>
				i === planIdx
					? { ...p, perks: p.perks.filter((_, j) => j !== perkIdx) }
					: p,
			),
		);
	};
				...
				<div className="flex items-start justify-between gap-4">
					<div>
						...
						<p className="text-sm text-(--secondary) mt-0.5">
							Basic identity details are managed in Settings; profile media can
							be edited here.
						</p>
					</div>
					<Button
					...
					<div className="rounded-xl border border-(--border) bg-(--surface-hover) p-4">
						<div className="flex flex-col items-center text-center gap-3">
							...
							<div className="w-full">
								<label className="block text-xs font-medium text-(--secondary) mb-1 text-left">
									Display Name
								</label>
								<input
									type="text"
					...
					<Card className="p-6 space-y-4">
						...
						{pricingPlans.length === 0 && (
							<div className="border border-dashed border-(--border) rounded-xl py-6 text-center">
								<p className="text-xs text-(--secondary)">
									No plans yet. Click Add to create your first plan.
								</p>
							</div>
						)}
							...
							>
								...
								<div>
									...
									<input
										...
										onChange={(e) =>
											updatePricingPlan(idx, "sessionType", e.target.value)
										}
										...
										{plan.perks.map((perk, perkIdx) => (
											<div key={perkIdx} className="flex items-center gap-1.5">
												<input
													...
													value={perk}
													onChange={(e) =>
														updatePerk(idx, perkIdx, e.target.value)
													}
