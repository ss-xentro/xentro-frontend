...
export default function RightSidebar({ onRequireAuth }: { onRequireAuth: () => void }) {
	return (
		<aside className="sticky top-0 h-screen w-60 shrink-0 hidden xl:block p-4 overflow-y-auto overflow-x-hidden">
					...
					<div className="space-y-4">
						{suggestions.map((user) => (
							<div key={user.username} className="flex items-center gap-3">
								...
								<button onClick={onRequireAuth} className="px-4 py-1.5 bg-white text-[#0B0D10] rounded-full text-sm font-semibold hover:bg-gray-200 transition-colors">
									Follow
								</button>
							</div>
						))}
