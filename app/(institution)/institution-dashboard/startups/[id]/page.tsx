					...
					<div className="flex items-start gap-5 mb-4">
						...
						<div className="flex-1">
							<div className="flex items-center gap-3 mb-1">
								...
								{startup.stage && (
									<span className="px-2 py-1 text-xs font-medium bg-blue-500/20 text-blue-600 dark:text-blue-200 rounded">
										{stageLabels[startup.stage] || startup.stage}
									</span>
								)}
								{startup.status && (
									<span className={`px-2 py-1 text-xs font-medium rounded ${startup.status === 'public' ? 'bg-green-500/20 text-green-600 dark:text-green-200' : 'bg-(--accent-light) text-(--primary-light)'}`}>
										{startup.status}
									</span>
								)}
							</div>
							...
							<div className="mb-6">
								...
								<div className="flex flex-wrap gap-2">
									{startup.sectors.map((s, i) => (
										<span key={i} className="px-3 py-1 text-sm font-medium bg-blue-500/20 text-blue-600 dark:text-blue-200 rounded-full">{s}</span>
									))}
								</div>
							</div>
						)}
							...
							<div>
								...
								<div className="flex flex-wrap gap-2">
									{startup.sdgs.map((s, i) => (
										<span key={i} className="px-3 py-1 text-sm font-medium bg-green-500/20 text-green-600 dark:text-green-200 rounded-full">{s}</span>
									))}
								</div>
							</div>
						)}
