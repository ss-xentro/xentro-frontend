...
export default function StartupCard({ startup, deleting, onDelete }: StartupCardProps) {
	...
	return (
		<Card
			className="p-6 bg-(--accent-subtle) border border-(--border) hover:border-(--border-hover) transition-colors cursor-pointer"
		...
		>
			...
			{startup.oneLiner && (
				<p className="text-sm text-(--secondary) line-clamp-3">{startup.oneLiner}</p>
			)}
