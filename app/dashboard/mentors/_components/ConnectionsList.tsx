		...
		<>
			...
			{filtered.length === 0 ? (
				<div className="flex flex-col items-center justify-center py-16 text-center">
					...
					<Link href="/explore/mentors" className="text-sm text-accent hover:underline">Explore Mentors →</Link>
				</div>
			) : (
				<div className="space-y-3">
					{filtered.map((conn) => {
						...
						return (
							<div key={conn.id} className="flex items-start gap-4 p-4 bg-(--surface) border border-(--border) rounded-xl hover:border-(--border-hover) transition-colors">
								...
								<div className="flex-1 min-w-0">
									...
									{conn.status === 'accepted' && (
										<div className="mt-3 flex gap-2">
											<Link href={`/explore/mentors/${conn.mentor}`} className="inline-flex items-center gap-1.5 px-3 py-1.5 text-xs font-medium rounded-lg bg-accent/10 text-accent hover:bg-accent/20 transition-colors">
												<svg className="w-3.5 h-3.5" fill="none" stroke="currentColor" viewBox="0 0 24 24" strokeWidth={2}>
													<path strokeLinecap="round" strokeLinejoin="round" d="M8 7V3m8 4V3m-9 8h10M5 21h14a2 2 0 002-2V7a2 2 0 00-2-2H5a2 2 0 00-2 2v12a2 2 0 002 2z" />
												</svg>
												Book Session
											</Link>
											<Link href={`/explore/mentors/${conn.mentor}`} className="inline-flex items-center gap-1.5 px-3 py-1.5 text-xs font-medium rounded-lg border border-(--border) text-(--secondary) hover:text-(--primary) hover:border-(--border-hover) transition-colors">
												View Profile
											</Link>
										</div>
									)}
