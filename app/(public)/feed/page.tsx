...
function PostCard({ post, isLast, onRequireAuth }: { post: Post; isLast?: boolean; onRequireAuth?: () => void }) {
...
export default function FeedPage() {
  ...
  // Dynamic navigation items based on user role — only shown to authenticated users
  ...
  // Gate function for interactions — prompts login for guests
              ...
              <div className="space-y-4">
                {suggestions.map((user) => (
                  <div key={user.username} className="flex items-center gap-3">
                    ...
                    <button onClick={() => requireAuth()} className="px-4 py-1.5 bg-white text-[#0B0D10] rounded-full text-sm font-semibold hover:bg-gray-200 transition-colors">
                      Follow
                    </button>
                  </div>
                ))}
