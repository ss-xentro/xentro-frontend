...
export default function LegalDocumentsSlide({
	formData,
	onChange,
}: LegalDocumentsSlideProps) {
	...
	const handleFileUpload = async (e: React.ChangeEvent<HTMLInputElement>) => {
		...
		if (remaining <= 0) {
			toastError(
				`Maximum ${MAX_DOCUMENTS} documents allowed. Remove some to upload more.`,
			);
			e.target.value = "";
			return;
		}
		if (files.length > remaining) {
			toastWarning(
				`Only ${remaining} more document${remaining > 1 ? "s" : ""} can be added. First ${remaining} will be uploaded.`,
			);
		}
		...
		if (oversizedFiles.length > 0) {
			toastError(
				`Files exceed ${MAX_FILE_SIZE_MB}MB limit: ${oversizedFiles.join(", ")}`,
			);
			e.target.value = "";
			return;
		}
		...
		try {
			...
			onChange({
				legalDocuments: [...(formData.legalDocuments || []), ...uploaded],
			});
			toastSuccess(
		...
		<div className="space-y-6 max-w-3xl mx-auto animate-fadeIn">
			<div className="text-center space-y-2 mb-8">
				...
				<p className="text-(--secondary) max-w-xl mx-auto">
					Upload legal documents that recognize your institution in India (e.g.,
					Certificate of Incorporation, Trust Deed, Society Registration, DPIIT
					Recognition, etc.)
				</p>
			</div>
				...
				<div className="flex items-start gap-3">
					...
					<div className="flex-1">
						...
						<p className="text-blue-800 text-sm mt-2">
							<strong>Format:</strong> PDF only (Max 10MB per file, up to{" "}
			...
			{formData.legalDocuments && formData.legalDocuments.length > 0 && (
				<div className="space-y-3">
					<h3 className="font-semibold text-(--primary)">
						Uploaded Documents ({formData.legalDocuments.length}/{MAX_DOCUMENTS}
						)
					</h3>
					{formData.legalDocuments.map((doc, index) => {
						...
						return (
							<div
								key={index}
								className="flex items-center gap-3 p-4 bg-(--surface-hover) rounded-lg"
							>
								<svg
			...
			<div className="bg-yellow-50 border border-yellow-200 rounded-lg p-4">
				<p className="text-yellow-900 text-sm flex items-start gap-2">
					...
					<span>
						These documents are required for verification. They will be reviewed
						by our team and kept confidential. Ensure all documents are clear
						and legible.
					</span>
				</p>
			</div>
